...
function M.to_string2(money, exact, color)
	...
	local settings = aux.character_data.tooltip
	local fmt = START
	if g > 0 then
		str = str .. format(fmt, GOLD, g) .. (settings.unit_cn and GOLD_TEXT_CN or GOLD_TEXT_EN)
		fmt = PART
	end
	if s > 0 or c > 0 then
		str = str .. format(fmt, SILVER, s) .. (settings.unit_cn and SILVER_TEXT_CN or SILVER_TEXT_EN)
		fmt = PART
	end
	if c > 0 then
		str = str .. format(fmt, COPPER, c) .. (settings.unit_cn and COPPER_TEXT_CN or COPPER_TEXT_EN)
	end
	if str == '' then
		str = NONE
	end
...
function M.to_string(money, pad, trim, color, no_color)
	...
	if no_color then
		...
		gold_text, silver_text, copper_text = '|cffffd70a'.. g ..'|r', '|cffc7c7cf'.. s ..'|r', '|cffeda55f'.. c ..'|r'
	end
