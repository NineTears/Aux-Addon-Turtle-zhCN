...
function SlashCmdList.AUX(command)
	...
	elseif arguments[1] == 'show' and arguments[2] == 'hidden' then
		...
		aux.print('- post bid [' .. status(aux.account_data.post_bid) .. '] 在卖选项卡中添加出价清单')
		...
		aux.print('- tooltip value [' .. status(tooltip_settings.value) .. '] 鼠标提示 价值')
		aux.print('- tooltip daily [' .. status(tooltip_settings.daily) .. '] 鼠标提示 日常')
		...
		aux.print('- unitcn [' .. status(tooltip_settings.unit_cn) .. '] 中文货币单位')
		aux.print('- sharing [' .. status(aux.account_data.sharing) .. ']')
		aux.print('- show hidden [' .. status(aux.account_data.showhidden) .. ']')
    end
end